...
> Подводния камък при промнеливи от реален тип (float) е, че те не закръглят с точност. Тоест вместо стойността, която се запазва в float променлива да 3, тч ще се запази като 3.00000123

## 3. Пресмятания, аритметични и логически оператори, изрази
Преснятанията в един програмен език са извършват върху променливи
...
- Опреациите изпълняват в реда, както и в математиката

- Разлика между аритметични и логически оператори

> Аритметичните операции се извършват на десетично ниво. Примери за аритметични оператори са `+`, `-`, `*`, `/`, `%`. Тоест 10 + 10 = 20. Логическите оператори обаче оперират на бинарно ниво. Пример за логичеки оператори са `&`, `|`, `^`, `!`, `<<`, `>>`. Всичките тези оператори са бинарни, тоест оперират с 2 параметъра. Докато оператора `!` е унарен. Тоест изисква само един параметър
...
**б) Бинарни оператори**
Както и нормални операции има и бинарни такива
- Събиране
